...
const sendTokenResponse = (user, statusCode, res, req, message) => {
  ...
  res.status(statusCode).cookie("authToken", token, cookieOptions).json({
    success: true,
    message,
  ...
  try {
    ...
    if (req.body.token || req.body.authToken) {
      return res.status(403).json({
        success: false,
        message:
          "⛔ Security Violation: Do not send tokens in the request body.",
      });
    }
...
exports.createStaff = async (req, res) => {
  try {
    const { username, password, fullName, phone, email, permissions } =
      req.body;
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    const allStudents = await Student.find(
      {},
      "studentId barcodeId studentName",
    ).limit(5);
    console.log(
    ...
    const student = await Student.findOne({
      $or: [
        ...
        {
          _id:
            userId.match && userId.match(/^[0-9a-fA-F]{24}$/) ? userId : null,
        },
      ].filter((q) => q && Object.values(q)[0]),
    });

    console.log(
      `🔍 Student search result: ${student ? `Found ${student.studentName} (${student.barcodeId})` : "Not found"}`,
    );
    ...
    if (student) {
      ...
      console.log(
        `✅ Student password reset for: ${student.studentName} (${student.studentId})`,
      );
      return res.status(200).json({
        success: true,
