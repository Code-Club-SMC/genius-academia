...
interface AdmissionSlipProps {
  ...
  config?: {
    studentProfilePictureSettings?: {
      pictureDisplayOnSlip?: boolean;
      fallbackEmoji?: string;
    };
  };
}

export const AdmissionSlip = ({
  student,
  session,
  config,
}: AdmissionSlipProps) => {
  ...
  // Determine if we should display picture on slip
  const pictureSettings = config?.studentProfilePictureSettings;
  const shouldDisplayPicture = pictureSettings?.pictureDisplayOnSlip !== false; // Default true
  const fallbackEmoji = pictureSettings?.fallbackEmoji || "👤";

  // Get the profile picture URL (priority: profilePictureUrl > photo)
  const profilePhotoUrl = student.profilePictureUrl || student.photo;
    ...
    <div ref={printRef} className="print-slip hidden print:block">
      ...
      <div className="max-w-4xl mx-auto p-8 bg-white relative">
        ...
        <div className="content-wrapper">
          ...
          {/* Student ID Section - REPLACED WITH PROFILE PICTURE */}
          {shouldDisplayPicture ? (
            <div className="mb-8 bg-gradient-to-r from-sky-50 to-blue-50 hairline border border-slate-200 rounded-lg p-8">
              <div className="flex flex-col items-center justify-center">
                {/* Profile Picture Circle */}
                <div className="w-48 h-48 rounded-full overflow-hidden border-4 border-slate-300 shadow-lg flex items-center justify-center bg-gradient-to-br from-sky-100 to-blue-100 mb-6">
                  {profilePhotoUrl ? (
                    <img
                      src={
            ...
            // Original Student ID section (if picture display is disabled)
            <div className="mb-8 bg-gradient-to-r from-sky-50 to-blue-50 hairline border border-slate-200 rounded-lg p-5">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.1em] mb-1">
                    Student ID
                  </p>
          ...
          <div className="mb-8">
            <div className="hairline border border-slate-200 rounded-lg overflow-hidden">
              <div className="bg-slate-50 px-5 py-3 hairline border-b border-slate-200">
                <h2 className="text-sm font-bold text-gray-800 uppercase tracking-[0.1em]">
                  Student Profile
                </h2>
              </div>
              <div className="p-5">
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="hairline border-b border-gray-200">
                        <th className="text-left text-xs font-semibold text-gray-600 uppercase tracking-wide pb-2">
                          Subject
                        </th>
                        <th className="text-right text-xs font-semibold text-gray-600 uppercase tracking-wide pb-2">
                          Status
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      {student.subjects.map((subject, index) => (
                        <tr
                          key={index}
                          className={`hairline border-b border-gray-100 ${index % 2 === 0 ? "bg-slate-50" : "bg-white"}`}
                        >
                ...
                <div className="space-y-3">
                    ...
                    <>
                      <div className="flex justify-between items-center pb-3 hairline border-b border-gray-200">
                        ...
                        <span className="text-lg font-bold text-gray-900">
                          {student.sessionRate.toLocaleString()} PKR
                        </span>
                      </div>
                      {student.discountAmount && student.discountAmount > 0 && (
                        <div className="flex justify-between items-center pb-3 hairline border-b border-gray-200">
                          ...
                          <span className="text-lg font-bold text-green-700">
                            -{student.discountAmount.toLocaleString()} PKR
                          </span>
                        </div>
                      )}
                      <div className="flex justify-between items-center pb-3 hairline border-b border-gray-200">
                  ...
                  <div className="flex justify-between items-center pb-3 hairline border-b border-gray-200">
                    ...
                    <span className="text-lg font-bold text-green-600">
                      {student.paidAmount.toLocaleString()} PKR
                    </span>
                  </div>
          ...
          <div className="hairline border-t border-gray-300 pt-4">
            <div className="text-center space-y-1">
              ...
              <p className="text-xs text-gray-500">
                For inquiries, contact academy administration.
              </p>
